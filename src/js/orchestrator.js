...
4. Always prioritize accuracy, then speed, then cost.`,
  ...
  // Store last generated image so it can be displayed after the agentic loop
  _lastGeneratedImage: null,

  async callProviderImage(prompt) {
    const hasOpenAI = await this.hasProvider('openai');
    const hasStability = await this.hasProvider('stability');

    if (!hasOpenAI && !hasStability) return 'No image provider configured. Add an OpenAI or Stability AI key in Settings.';
    ...
    // Try DALL-E 3 first (better quality, uses existing OpenAI key)
    if (hasOpenAI) {
      try {
        this.emitStep('tool', `Generating image with DALL-E 3: "${prompt.substring(0, 50)}..."`);
        const result = await window.avis.apiCall({
          provider: 'dalle',
          model: 'dall-e-3',
          messages: [{ role: 'user', content: enhancedPrompt }],
          systemPrompt: '',
          options: { wallpaper: isWallpaper }
        });
        ...
        if (!result.error && result.image) {
          ...
          return `[Image generated by DALL-E 3]\nPrompt: "${prompt}"${revised}${isWallpaper ? '\nOptimized for desktop wallpaper.' : ''}`;
        }
        // DALL-E failed — fall through to Stability
    ...
    if (hasStability) {
      try {
        this.emitStep('tool', `DALL-E unavailable — trying Stability AI: "${prompt.substring(0, 50)}..."`);
        const result = await window.avis.apiCall({
          provider: 'stability',
        ...
        if (!result.error && result.image) {
          this._lastGeneratedImage = { data: result.image.data, mimeType: result.image.mimeType, prompt, isWallpaper };
          return `[Image generated by Stability AI (fallback)]\nPrompt: "${prompt}"`;
        }
        return `Image generation failed: ${result.error ? this.parseError(result.message) : 'No image returned'}`;
      } catch (err) {
        return `Image generation failed: ${err.message}`;
      }
    }

    return 'Image generation failed on all providers.';
  },
