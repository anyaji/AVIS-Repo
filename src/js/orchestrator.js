...
4. Always prioritize accuracy, then speed, then cost.`,
  ...
  // Document generation tools — only included when user asks for docs/presentations/spreadsheets
  ...
  PROVIDER_TOOLS: [
    ...
    },
    {
      ...
      input_schema: { type: "object", properties: { prompt: { type: "string", description: "Task to send to Gemini" }, model: { type: "string", enum: ["gemini-2.5-flash", "gemini-1.5-flash"], description: "Model to use (default: gemini-2.5-flash)" } }, required: ["prompt"] }
    },
    {
...
10. If provider fails twice \u2192 Claude handles directly`;
  ...
  async process(userMessage, files = []) {
    ...
    // Handle "remember X" commands — save to journal
    ...
    if (routing.agentKey !== 'claude' && files.length === 0 && !DelegationProtocol.shouldEscalate(routing.agentKey)) {
      ...
      if (hasAgent && this.isProviderAvailable(routing.agentKey)) {
        ...
        if (!isDelegationError) {
          ...
          // Delegation failed — record failure, fall through to Claude
          DelegationProtocol.recordFailure(routing.agentKey);
          this.emitStep('warn', `${routing.agent} delegation failed — falling back to Claude`, 'warn');
        }
      }
    }
  ...
  // Image generation — DALL-E 3 only
  _lastGeneratedImage: null,
  imageResolution: '1024x1024', // default, changeable via UI dropdown

  async callProviderImage(prompt) {
    if (!(await this.hasProvider('openai'))) return 'OpenAI key required for image generation. Add it in Settings.';

    const isWallpaper = /wallpaper|background|desktop bg/i.test(prompt);
    const enhancedPrompt = isWallpaper
      ? prompt + ', desktop wallpaper, high resolution, clean composition, suitable as desktop background'
      : prompt;

    // Pick resolution: wallpaper → landscape, else use user-selected or default
    let size = this.imageResolution || '1024x1024';
    if (isWallpaper) size = '1792x1024';

    try {
      this.emitStep('tool', `DALL-E 3 generating (${size}): "${prompt.substring(0, 50)}..."`);
      const result = await window.avis.apiCall({
        provider: 'dalle',
        model: 'dall-e-3',
        messages: [{ role: 'user', content: enhancedPrompt }],
        systemPrompt: '',
        options: { size, wallpaper: isWallpaper }
      });

      if (result.error) return `Image generation failed: ${this.parseError(result.message)}`;
      if (result.image) {
        this._lastGeneratedImage = { data: result.image.data, mimeType: result.image.mimeType, prompt, isWallpaper };
        const revised = result.revisedPrompt ? `\nDALL-E revised prompt: "${result.revisedPrompt}"` : '';
        return `[Image generated by DALL-E 3 at ${size}]\nPrompt: "${prompt}"${revised}`;
      }
